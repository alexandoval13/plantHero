...
const App = () => {
  ...
  const handleSubmitPlant = (e) => {
    ...
    var light =  (lightLevel && lightSource) ? (lightLevel + ', ' + lightSource) : ((lightLevel || lightSource) ? (lightLevel || lightSource) : null );
    var wateringCycleAmount = e.target['watering-cycle-amount'].value > 0 ? e.target['watering-cycle-amount'].value : null;
  ...
  const uploadImage = (e) => {
    ...
    axios.post('https://api.cloudinary.com/v1_1/dpbieru1u/image/upload', data)
    .then( (res) => {
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['sidebar']} >
        <Sidebar setContent={setContent} setFilter={setFilter} categories={categories} setPlant={setPlant} />
      </div>
      ...
      <div className={styles.content}>
        ...
        { (content === 'PlantList') ? <PlantList date={date} handleWater={handleWater} plants={plantsData} filter={filter} setPlant={setPlant} setContent={setContent} handleMouse={handleMouse} /> : null }

        <Info selectedPlant={selectedPlant} setPlant={setPlant} content={content} />

        { (content === 'Form') ? <Form handleSubmitPlant={handleSubmitPlant} uploadImage={uploadImage} setImage={setImage} setContent={setContent} image={image} categories={categories} /> : null }
        { (content === 'Confirmation') ? <Confirmation setContent={setContent} newPlant={newPlant} setPlant={setPlant} setFilter={setFilter} /> : null }
      </div>
    </div>
  )
}
